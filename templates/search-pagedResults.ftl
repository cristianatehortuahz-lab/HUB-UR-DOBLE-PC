...
${stylesheets.add('<link rel="stylesheet" type="text/css" href="${urls.base}/themes/wilma/css/pure_dashboard_hub_v23.css"/>')}
...
<div class="contentsBrowseGroup" id="pure-dashboard-wrapper">
    ...
    <header id="dashboard-top-header">
        <meta name="description" content="Encuentra investigadores, programas académicos y producción científica en el HUB de Investigación de la Universidad del Rosario.">
...
<#-- Paging controls -->
    <#if (pagingLinks?size > 0)>
        <nav class="searchpages" aria-label="Navegaci&oacute;n de resultados">
            ${i18n().pages}:
            <#if prevPage??><a class="prev" href="${prevPage?html}" title="${i18n().previous}" aria-label="P&aacute;gina anterior">${i18n().previous}</a></#if>
            <#list pagingLinks as link>
                <#if link.url??>
                    <a href="${link.url?html}" title="${i18n().page_link}" aria-label="Ir a la p&aacute;gina ${link.text?html}">${link.text?html}</a>
                <#else>
                    <span aria-current="page" class="current-page">${link.text?html}</span> <#-- no link if current page -->
                </#if>
            </#list>
            <#if nextPage??><a class="next" href="${nextPage?html}" title="${i18n().next_capitalized}" aria-label="P&aacute;gina siguiente">${i18n().next_capitalized}</a></#if>
        </nav>
    </#if>
</#macro>
